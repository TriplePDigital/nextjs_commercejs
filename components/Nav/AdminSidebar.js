...
const AdminSidebar = () => {
	...
	return (
		<aside className="h-full bg-gray-50 w-full md:w-2/12 mt-5 rounded flex-col shadow-md border relative">
			<Link href="/admin/">
			...
			{showQuizChildren ? (
				<ul className="text-white w-full">
					<Link href="/admin/quiz?tabIndex=0" passHref={false}>
						<a
							className={`w-full block hover:bg-gray-700 px-3 py-2 border-gray-300 cursor-pointer border-b ${
								router.query.tabIndex === '0' &&
								router.pathname.includes('/admin/quiz')
									? 'bg-gray-700'
									: 'bg-gray-800'
							}`}
						>
					...
					<Link href="/admin/quiz?tabIndex=1" passHref={false}>
						<a
							className={`w-full block hover:bg-gray-700 px-3 py-2 border-gray-300 cursor-pointer border-b ${
								router.query.tabIndex === '1' &&
								router.pathname.includes('/admin/quiz')
									? 'bg-gray-700'
									: 'bg-gray-800'
							}`}
			...
			{showEnrollmentChildren ? (
				<ul className="bg-gray-800 text-white w-full">
					<Link href="/admin/enrollment?tabIndex=0" passHref={false}>
						<a
							className={`w-full block hover:bg-gray-700 px-3 py-2 border-gray-300 cursor-pointer border-b ${
								router.query.tabIndex === '0' &&
								router.pathname.includes('/admin/enrollment')
									? 'bg-gray-700'
									: 'bg-gray-800'
							}`}
						>
					...
					<Link href="/admin/enrollment?tabIndex=1" passHref={false}>
						<a
							className={`w-full block hover:bg-gray-700 px-3 py-2 border-gray-300 cursor-pointer border-b ${
								router.query.tabIndex === '1' &&
								router.pathname.includes('/admin/enrollment')
					...
					<Link href="/admin/enrollment?tabIndex=2" passHref={false}>
						<a
							className={`w-full block hover:bg-gray-700 px-3 py-2 border-gray-300 cursor-pointer ${
								router.query.tabIndex === '2' &&
								router.pathname.includes('/admin/enrollment')
									? 'bg-gray-700'
									: 'bg-gray-800'
							}`}
