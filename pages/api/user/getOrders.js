...
export default async function (req, res) {
    if (req.method !== 'GET') {
        ...
        const payment = await axios.post(
            `https://sweetblinginc.transactiongateway.com/api/query.php?security_key=${process.env.NMI_SECRET_KEY}&email=${email.toLowerCase().trim()}`
        )
        ...
        if (!transaction) {
            res.status(400)
            res.json({message: 'Transaction not found', transaction: []})
        } else {
